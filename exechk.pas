...
Procedure AnalyzeExeFile;
	Var
        ...
        Writeln;
        AnalOut('Checkword (header)',CheckSum);
        CalcSum:=0;
        ...
        AnalOut('Checkword should be		',CheckSum);
        Writeln;
        If CalcSum=$FFFF Then
        	Writeln(ExeFileName, ' has correct checkword in header.') Else
        Begin
        ...
        If Ch in ['Y','y'] Then
        	Begin
                Reset (ExeFile,$1B);
                BlockWrite (ExeFile,ExeHeader,1);
                Close(ExeFile);
                Writeln('exesum: ', ExefileName, ' has been fixed');
        	End;
        End;
End;
End;
Begin
	Writeln('Pure masochistic EXE Header and Checksum Utility Version 1.0');
        ...
        End Else
        Begin
        Writeln('exechk: ', ExeFileName, ': No such file or directory');
        Halt(1);
        End;
